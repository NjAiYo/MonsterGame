...
GaussianBlurFilter* GaussianBlurFilter::create()
{
...
GaussianBlurFilter* GaussianBlurFilter::create(float param)
{
    GaussianBlurFilter* filter = GaussianBlurFilter::create();
    filter->setParameter(param);
    return filter;
}
...
: _resolation(800.f)
{
    this->shaderName = kCCFilterShader_gaussian_blur;
}

GLProgram* GaussianBlurFilter::loadShader()
{
    GLProgram* p = GLProgram::createWithByteArrays(ccPositionTextureColor_noMVP_vert, ccFilterShader_gaussian_blur_frag);
    //	GLProgram* p = new GLProgram();
...
void GaussianBlurFilter::setUniforms(GLProgram* cgp)
{
...
void GaussianBlurFilter::initSprite(FilteredSprite* sprite)
{
